...
### 出力フレーム（16:9固定・カバーフィット）

レンダラー・カメラのフラスタムは常に16:9固定（`OUTPUT_ASPECT` in `main.js`）。
写真の縦横比がそれと違っても、写真自体は歪ませず（メッシュのスケールは常に等倍）、
16:9のフレームを完全に覆うまで拡大してから、はみ出した分をオルソカメラのフラスタム外
としてクロップする（横長の写真は左右が、縦長の写真は上下がクロップされる）。
ブラウザのウィンドウ/パネル幅が16:9と違う形でも、レンダラーのサイズ自体をその中に
収まる最大の16:9矩形に決めるので（`resize()`）、はみ出た分は`#viewport`の黒帯になる。
