...
- [x] **Phase 1 — 輝度マップによる明滅の最小構成**（実装済み・実機での確認待ち）: `runtime/` に写真を読み込むと輝度マップを自動生成し、MIDI Note Onに応じて写真そのものの明るさを上げ下げする（個別の光点は打たない）。手動でマップの元データを作りたい場合は `mask-editor/`（任意）。
- [x] **Phase 2 — 深度による3D化**（実装済み・実験的）: 本物の深度推定モデルはまだ無いため、画面のY位置＋輝度から作る疑似深度でディスプレイスメントとカメラ移動（dolly_in）を実装。本物の深度マップへの差し替えと振幅の実測は今後
...
検出した場所は最大4クラスタ（0-3）に振り分けられ、それぞれ `config/midi-mapping.json` の
同じ `cluster` 番号を持つトラックに連動する。

1. **公開URL の `runtime/` を開く**
   - 左パネルで夜景写真を読み込む（写真がまだ無ければ「サンプルを読み込む」でも可）。
     または「Dropbox」パネルからDropboxアカウントでログインし、指定したアプリ専用フォルダ
     （`Apps/<アプリ名>/`）内の写真をまとめて自動スライドショー表示することもできる
     （一定時間ごとの自動切り替えと、`role: scene_cut`＝SAMPLERトラックのMIDI Note Onでの
     切り替えの両方に対応）。スマホのDropboxアプリでそのフォルダに写真を移動/コピーして
     おけば、そのまま表示対象になる。初回ログイン後は次回以降クリック無しで自動的に同じ
     フォルダが再読み込みされる（Google Photosは共有アルバムAPIの廃止・一括選択機能の
     欠如・選択の期限切れが重なり断念した。詳細は `runtime/README.md`「Dropbox連携」参照）
   ...
   - SEQTRAKで各トラックを鳴らすと、そのトラックの `cluster` 番号に対応するマップ上の場所が
     指数減衰（`tau`）で明るさが上がる（`config/midi-mapping.json` の `name: "KICK"` は
     cluster0固定）
   - 「Cluster Envelopes」のメーターでクラスタごとのMIDIエンベロープを確認できる
   ...
   - 「Depth 3D」パネル（Phase 2・実験的）で、疑似深度によるディスプレイスメントと
     カメラの前後移動（dolly_in）を確認・調整できる。本物の深度推定はまだ無いため、
     写真のY位置と輝度から作る簡易的な深度で代用している
